...
This file documents researched source options and data lineage decisions for the first milestone. Raw downloads are tracked separately in `data/raw/source_manifest.csv` after `scripts/01_download_sources.py` is run.
...
- `scripts/01_download_sources.py` downloads the selected INE population workbook and the selected Geoportal DPA ZIP into `data/raw/`, without opening, cleaning, unzipping, filtering, or transforming the files.
